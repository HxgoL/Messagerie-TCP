...
void afficherListeChainee(char *resp){
    pthread_mutex_lock(&mutex);

    client_data* c = first;
    int i = 1;

    resp[0] = '\0'; // Vide resp au départ

    while (c != NULL) {
        char ligne[128];
        snprintf(ligne, sizeof(ligne), "Client numéro %d, sock=%d\n", i, c->sock);
        strncat(resp, ligne, BUFSIZE - strlen(resp) - 1); // Ajoute sans dépasser BUFSIZE
        c = c->next;
        i++;
    }
